...
- `js/game.js` — `gameState` (single source of truth: `appReady`, `nearestLink`, `selectedLinkIndex`, `accelerationFactor`, `warpActive`), p5 hooks (`setup`, `draw`, `keyPressed`, `windowResized`), input/game logic, warp/autopilot (`startWarp()`, `endWarp(silent)`, `updateWarp()`, screen-space `warpStreaks`), trash collection (`trash` array, `trashCollected`, `collectTrash()`), visited links (session-only Set), mini-map, and the boot guard.
