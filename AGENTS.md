...
- `js/classes.js` — `Star`, `Ship`, `Link`, `Navigator`, `Planet`, `Nebula` (drifting gas clouds, `blendMode(SCREEN)`), `Asteroid` (tumbling rocks), `ThrusterParticle` (retro exhaust), `Trash` (collectibles; optional `assets/trash/` PNG/GIF sprites via `loadTrashImages()`, procedural pixel-art fallback).
...
- `js/game.js` — `gameState` (single source of truth: `appReady`, `nearestLink`, `selectedLinkIndex`, `accelerationFactor`, `warpActive`), p5 hooks (`setup`, `draw`, `keyPressed`, `windowResized`), input/game logic, warp/autopilot (`startWarp()`, `endWarp(silent)`, `updateWarp()`, screen-space `warpStreaks`), the floating `[ENTER] Open` / `[V] Preview` prompt above the UFO (`drawShipPrompt()`), trash collection (`trash` array, `trashCollected`, `collectTrash()`), visited links (session-only Set), mini-map, and the boot guard.
